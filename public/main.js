...
const stopGame = () => {
  ...
  reloadButtonElement.classList.add('hidden');
  ...
  allDeletedElement.innerHTML = allDeletedSsrbs + "";
  ...
  if (touches.length === 1) { // 2か所以上タッチしていない状態でないと処理をしないようにする
    ...
    if (
      ...
      if (selectedSsrbs.length >= 7) {
        chainElement.innerHTML = '<span style="font-size: 30px;" class="gaming anime_text">' + selectedSsrbs.length + "</span>  HITS!";
      } else if (selectedSsrbs[0][1].label === "goldenBall") {
        chainElement.innerHTML = '<span style="font-size: 30px;" class="golden anime_text">' + selectedSsrbs.length + "</span>  HITS!";
      } else {
        chainElement.innerHTML = '<span style="font-size: 30px;" class="anime_text">' + selectedSsrbs.length + "</span>  CHAINS!";
      }
      ...
      if (selectedSsrbs[0][1].label === "goldenBall") {
        ...
        playMusic(ohFemalePath)
        playMusic(crap1Path)
        commentElement.innerHTML = '<span class="gaming anime_text" style="letter-spacing:1px; font-size: 24px;">WONDERFUL!</span>';
      } else if (selectedSsrbs.length === 10) {
        playMusic(ohFemalePath)
        playMusic(crap1Path)
        commentElement.innerHTML = '<span class="gaming anime_text" style="letter-spacing:1px; font-size: 24px;">BRILLIANT!</span>';
      } else if (selectedSsrbs.length === 11) {
        playMusic(ohMalePath)
        playMusic(crap2Path)
        commentElement.innerHTML = '<span class="gaming anime_text" style="letter-spacing:2px; font-size: 24px;">BRILLIANT!</span>';
      } else if (selectedSsrbs.length === 12) {
        playMusic(ohMalePath)
        playMusic(crap2Path)
        commentElement.innerHTML = '<span class="gaming anime_text" style="letter-spacing:2px; font-size: 24px;">FABULOUS!</span>';
      ...
      chainElement.classList.remove('hidden')
      chainElement.animate(
        {
          transform: [
            'translateY(0px)',
            'translateY(0px)',
...
document.addEventListener("touchmove", function(events) {
  ...
  document.addEventListener('touchmove', () => events.preventDefault(), { passive: false });
  document.addEventListener('mousewheel', () => events.preventDefault(), { passive: false });
      ...
      for (let i = 0; i < engine.world.bodies.length; i++) {
        ...
        if (rad >= distance && (body.label === "ball" || body.label === "goldenBall")) {
          ...
          if (selectedSsrbs.length > 0 && targets.length > 0) {
            ...
            if (distance2 < (rad + prevSsrb[1].circleRadius) * 1.3) {
